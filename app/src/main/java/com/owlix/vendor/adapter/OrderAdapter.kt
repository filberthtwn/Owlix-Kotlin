...
): RecyclerView.Adapter<OrderAdapter.ViewHolder>(), OrderInterface {
...
//        return ViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_my_order, parent, false))
    ...
    class ViewHolder(val binding: OrderItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(order: Order, context: Context) {
            ...
            val datetime = DateFormatter.format("dd MMM yyyy, HH:mm", TimeZone.getDefault(), parser.parse(order.createdAt))
            binding.tvOrderDatetime.text = datetime
...
////                val action = HomeFragmentDirections.actionHomeToInputDiscount(offer)
////                fragment.findNavController().navigate(action)
//            }
//
////            when (offer.status) {
